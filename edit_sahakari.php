<?php
// edit_sahakari.php — Edit Sahakari Members (stores in sahakari_* tables)
// Based on edit_member.php with edit functionality
...
function validate_doc_by_type($type, $number) {
    ...
    switch ($t) {
        ...
        case 'other':            return strlen($n) >= 4;
        default:                 return strlen($n) >= 4;
    }
}
...
/* ----------- Ensure Sahakari tables exist ----------- */
function createSahakariTablesIfNotExist($conn) {
    try {
        // sahakari_members
        $check_members = $conn->query("SHOW TABLES LIKE 'sahakari_members'");
        if ($check_members->num_rows == 0) {
            $members_table = "CREATE TABLE sahakari_members (
            ...
            if (!$conn->query($members_table)) {
                throw new Exception("Error creating sahakari_members table: " . $conn->error);
            }
            $conn->query("ALTER TABLE sahakari_members ADD CONSTRAINT fk_sahakari_members_mahal 
                          FOREIGN KEY (mahal_id) REFERENCES register(id) 
                          ON DELETE CASCADE ON UPDATE CASCADE");
        } else {
            // add legacy/missing columns if needed
            $check_father = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'father_name'");
            if ($check_father->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN father_name VARCHAR(255) DEFAULT NULL AFTER head_name");
            }

            $check_mdd = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'monthly_donation_due'");
            if ($check_mdd->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN monthly_donation_due VARCHAR(20) DEFAULT 'pending' AFTER total_family_members");
            }
            $check_total_due = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'total_due'");
            if ($check_total_due->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN total_due DECIMAL(10,2) DEFAULT 0.00 AFTER monthly_donation_due");
            }
            $check_monthly_fee = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'monthly_fee'");
            if ($check_monthly_fee->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN monthly_fee DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER total_due");
            }
            // member_number column
            $check_member_number = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'member_number'");
            if ($check_member_number->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN member_number INT UNSIGNED DEFAULT NULL AFTER mahal_id");
            }
            // status column
            $check_status = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'status'");
            if ($check_status->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN status VARCHAR(20) DEFAULT 'active' AFTER gender");
            }
            // bulk_print_enabled column (default false)
            $check_bulk = $conn->query("SHOW COLUMNS FROM sahakari_members LIKE 'bulk_print_enabled'");
            if ($check_bulk->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD COLUMN bulk_print_enabled TINYINT(1) NOT NULL DEFAULT 0 AFTER monthly_fee");
            }
            // unique index on (mahal_id, member_number)
            $idxRes = $conn->query("SHOW INDEX FROM sahakari_members WHERE Key_name = 'uniq_sahakari_mahal_member_number'");
            if (!$idxRes || $idxRes->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_members ADD UNIQUE KEY uniq_sahakari_mahal_member_number (mahal_id, member_number)");
            }
        }

        // sahakari_family_members
        $check_family = $conn->query("SHOW TABLES LIKE 'sahakari_family_members'");
        if ($check_family->num_rows == 0) {
            $family_table = "CREATE TABLE sahakari_family_members (
                id INT AUTO_INCREMENT PRIMARY KEY,
            ...
            if (!$conn->query($family_table)) {
                throw new Exception("Error creating sahakari_family_members table: " . $conn->error);
            }
            $conn->query("ALTER TABLE sahakari_family_members ADD CONSTRAINT fk_sahakari_family_member 
                 FOREIGN KEY (member_id) REFERENCES sahakari_members(id) 
                 ON DELETE CASCADE ON UPDATE CASCADE");
        } else {
            // ensure father_name column exists
            $check_father_col = $conn->query("SHOW COLUMNS FROM sahakari_family_members LIKE 'father_name'");
            if ($check_father_col->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_family_members ADD COLUMN father_name VARCHAR(255) DEFAULT NULL AFTER name");
            }
            // ensure status column exists
            $check_status = $conn->query("SHOW COLUMNS FROM sahakari_family_members LIKE 'status'");
            if ($check_status->num_rows == 0) {
                $conn->query("ALTER TABLE sahakari_family_members ADD COLUMN status VARCHAR(20) DEFAULT 'active' AFTER email");
            }
        }

        // sahakari_member_documents
        $check_docs = $conn->query("SHOW TABLES LIKE 'sahakari_member_documents'");
        if ($check_docs->num_rows == 0) {
            $docs_table = "CREATE TABLE sahakari_member_documents (
                id INT AUTO_INCREMENT PRIMARY KEY,
                member_id INT NOT NULL,
            ...
            if (!$conn->query($docs_table)) {
                throw new Exception("Error creating sahakari_member_documents table: " . $conn->error);
            }
            $conn->query("ALTER TABLE sahakari_member_documents
                          ADD CONSTRAINT fk_sahakari_docs_member
                          FOREIGN KEY (member_id) REFERENCES sahakari_members(id)
                          ON DELETE CASCADE ON UPDATE CASCADE");
            $conn->query("ALTER TABLE sahakari_member_documents
                          ADD CONSTRAINT fk_sahakari_docs_family
                          FOREIGN KEY (family_member_id) REFERENCES sahakari_family_members(id)
                          ON DELETE CASCADE ON UPDATE CASCADE");
        }
...
/* --------------------------- API (Update) --------------------------- */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_sahakari_member') {
    ...
    try {
        $db_result = get_db_connection();
        if (isset($db_result['error'])) throw new Exception("Database connection failed: " . $db_result['error']);
        $conn = $db_result['conn'];
        ...
        $check = $conn->prepare("SELECT id, address, join_date, head_name FROM sahakari_members WHERE id = ? AND mahal_id = ?");
        $check->bind_param("ii", $member_id, $mahal_id);
        $check->execute();
        $res = $check->get_result();
        $exists = $res->fetch_assoc();
        $check->close();
        if (!$exists) throw new Exception('Member not found or access denied.');
        ...
        if ($head_father_name !== null && $head_father_name !== '' && strlen($head_father_name) > 255) throw new Exception('Father name is too long.');
        if ($head_email === '' || !filter_var($head_email, FILTER_VALIDATE_EMAIL)) throw new Exception('A valid email address is required.');
        ...
        if ($head_dob && !not_future_date($head_dob)) throw new Exception('Head DOB cannot be in the future.');
        $head_gender = isset($_POST['head_gender']) && $_POST['head_gender'] !== '' ? $_POST['head_gender'] : null;
        if (!$head_gender) throw new Exception('Gender is required.');
        $head_occupation = isset($_POST['head_occupation']) && $_POST['head_occupation'] !== '' ? trim($_POST['head_occupation']) : null;
        if (!$head_occupation) throw new Exception('Occupation is required.');
        $join_date = isset($_POST['join_date']) && $_POST['join_date'] !== '' ? $_POST['join_date'] : null;
        if (!$join_date) throw new Exception('Join date is required.');
        ...
        if (!is_numeric($monthly_fee) || $monthly_fee < 0) throw new Exception('Monthly fee must be a number ≥ 0.');
        ...
        if ($member_number < 1) throw new Exception('Member number must be at least 1.');
        ...
        if (isset($_POST['head_documents'])) {
            ...
            if (is_array($decoded_hd)) $head_documents = $decoded_hd;
        }
        ...
        if (isset($_POST['family_members'])) {
            ...
            if (is_array($decoded)) $family_members = $decoded;
        }
        $total_members = count($family_members) + 1;

        // Uniqueness for (mahal_id, member_number) excluding this member
        $chk = $conn->prepare("SELECT COUNT(*) AS c FROM sahakari_members WHERE mahal_id = ? AND member_number = ? AND id <> ?");
        ...
        $sql = "UPDATE sahakari_members SET 
                    head_name = ?, 
                    father_name = ?,
                    email = ?, 
                    phone = ?, 
        ...
        $del = $conn->prepare("DELETE FROM sahakari_family_members WHERE member_id = ?");
        ...
        if (!empty($family_members)) {
            ...
            $family_sql = "INSERT INTO sahakari_family_members (member_id, name, father_name, relationship, dob, gender, phone, email, status, created_at, updated_at)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
            $family_stmt = $conn->prepare($family_sql);
            if (!$family_stmt) throw new Exception("Prepare failed for family members: " . $conn->error);
        ...
        $deld = $conn->prepare("DELETE FROM sahakari_member_documents WHERE member_id = ?");
        ...
        if (!$doc_stmt) throw new Exception("Prepare failed for documents: " . $conn->error);
        ...
        foreach ($head_documents as $d) {
            ...
            $doc_number = isset($d['doc_number']) ? trim($d['doc_number']) : '';
            if (!$doc_type && !$doc_number) continue;
            if ($doc_type === '' || $doc_number === '') throw new Exception("Head document requires both type and number.");
            $owner_type = 'head';
        ...
        foreach ($family_members as $idx => $fm) {
            ...
            foreach ($fm['documents'] as $d) {
                ...
                $doc_number = isset($d['doc_number']) ? trim($d['doc_number']) : '';
                if (!$doc_type && !$doc_number) continue;
                if ($doc_type === '' || $doc_number === '') throw new Exception("Family document requires both type and number.");
                $owner_type = 'family';
                $name_on_doc = $nn(isset($d['name_on_doc']) ? trim($d['name_on_doc']) : null);
                $issued_by   = $nn(isset($d['issued_by']) ? trim($d['issued_by']) : null);
                $issued_on   = $nn(isset($d['issued_on']) ? trim($d['issued_on']) : null);
        ...
        if ($doc_stmt) { $doc_stmt->close(); }

        $conn->commit();

        echo json_encode([
            'success' => true,
            'message' => "Sahakari Member '{$head_name}' updated successfully!",
            'member_id' => $member_id
        ]);
        exit;
        ...
        if (isset($doc_stmt) && $doc_stmt) { @$doc_stmt->close(); }
        if (isset($family_stmt) && $family_stmt) { @$family_stmt->close(); }
...
try {
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        if (!isset($family_documents_by_id[$fid])) $family_documents_by_id[$fid] = [];
    ...
    error_log("Edit sahakari member fetch error: " . $e->getMessage());
    header("Location: sahakari_management.php");
    exit();
}
?>
...
<title>Edit Sahakari Member - <?php echo htmlspecialchars($member['head_name']); ?></title>
...
.logout-btn:hover {
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .badge{
        ...
        border:1px solid var(--border-form);background:#fff;margin-top:8px;color:#374151
    }
    .badge.success{border-color:#a7f3d0;background:#ecfdf5;color:#065f46}
    ...
    <div id="app">
        <aside class="sidebar" id="sidebar" aria-hidden="true" aria-label="Main navigation">
            ...
            <div class="sidebar-inner">
                ...
                <nav class="menu" role="menu">
                    ...
                    <button class="menu-btn active" id="sahakari-manage-btn" type="button">
                        <i class="fas fa-hand-holding-usd"></i>
                        <span>Sahakari Management</span>
                    </button>
        ...
        <main class="main" id="main">
            <div class="top-row">
                ...
                <div class="app-title">Edit Sahakari Member — <?php echo htmlspecialchars($member['head_name']); ?></div>
                <a href="sahakari_details.php?id=<?php echo (int)$member['id']; ?>" class="btn btn-primary-form">← Back to Member</a>
            </div>

            <div class="container">
                <div id="alertBox" class="alert"></div>
                <div id="loadingBox" class="loading"><p>✏️ Updating sahakari member data...</p></div>

                <form id="sahakariForm" method="POST" novalidate>
                    <input type="hidden" name="action" value="update_sahakari_member">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" name="head_name" id="head_name" value="<?php echo htmlspecialchars($member['head_name']); ?>" required maxlength="120">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="head_father_name" id="head_father_name" value="<?php echo htmlspecialchars($member['father_name'] ?? ''); ?>" required maxlength="255">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="member_number" id="member_number"
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" name="head_email" id="head_email" value="<?php echo htmlspecialchars($member['email']); ?>" required maxlength="120" inputmode="email" autocomplete="email">
                                <div class="error-message" id="head_email_error">Valid email required (must contain @ and .com/.in/.org etc.)</div>
                            </div>
                            <div class="form-group">
                                <label>Phone Number <span class="required">*</span></label>
                                <input type="tel" name="head_phone" id="head_phone" value="<?php echo htmlspecialchars($member['phone']); ?>" required
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="head_dob" id="head_dob" value="<?php echo $member['dob'] ? htmlspecialchars($member['dob']) : ''; ?>" required>
                            ...
                            <div class="form-group">
                                ...
                                <select name="member_status" id="member_status" required>
                                    <option value="" <?php echo empty($member['status']) ? 'selected' : ''; ?>>Select Status</option>
                                ...
                                <div class="help">"Frozen" status continues due generation. "Deceased" and "Terminated" stop all activities.</div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group full">
                                ...
                                <textarea name="head_address" id="head_address" required minlength="10"><?php echo htmlspecialchars($member['address']); ?></textarea>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="join_date" id="join_date" value="<?php echo htmlspecialchars($member['join_date']); ?>" required>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                <label>Monthly Donation Status <span class="required">*</span></label>
                                <select name="monthly_donation_due" id="monthly_donation_due" required>
                                    <option value="">Select Status</option>
                                    <option value="cleared" <?php echo ($member['monthly_donation_due']==='cleared')?'selected':''; ?>>Cleared</option>
                            ...
                            <div class="form-group">
                                <label>Total Due Amount (₹) <span class="required">*</span></label>
                                <input type="number" name="total_due" id="total_due" step="0.01" min="0" value="<?php echo htmlspecialchars(number_format((float)$member['total_due'], 2, '.', '')); ?>" required>
                                <div class="error-message" id="total_due_error">Must be a positive number</div>
                            </div>
                            <div class="form-group">
                                <label>Monthly Fee (₹ / month) <span class="required">*</span></label>
                                <input type="number" name="monthly_fee" id="monthly_fee" step="0.01" min="0" value="<?php echo htmlspecialchars(number_format((float)($member['monthly_fee'] ?? 0), 2, '.', '')); ?>" required>
                            ...
                            <div class="form-group">
                                <label class="checkbox-inline" style="display:flex;align-items:center;gap:8px;">
                                    <input type="checkbox" name="bulk_print_enabled" id="bulk_print_enabled" value="1" <?php echo !empty($member['bulk_print_enabled']) ? 'checked' : ''; ?>>
                                    Enable Bulk Print
                                </label>
                                <div class="help">If checked, this member will be included in bulk print operations. Default is OFF.</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="actions">
                        <div class="actions-left">
                            <a href="sahakari_details.php?id=<?php echo (int)$member['id']; ?>" class="btn btn-secondary-form">Cancel</a>
                            <button type="button" class="btn btn-secondary-form" id="clearBtn">Reset Changes</button>
                        </div>
                        <div class="actions-right">
                            <button type="submit" class="btn btn-success-form">Update Sahakari Member</button>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function validateDonationStatus(status) {
        return status !== '';
    }
    ...
    function setupAllValidations() {
        // Head of family validations - ALL REQUIRED
        setupInlineValidation('head_name', validateName, 'head_name_error', 'Head name is required (only letters and spaces allowed, min 2 characters)!', true);
        setupInlineValidation('head_father_name', validateFatherName, 'head_father_name_error', 'Father\'s name is required (only letters and spaces allowed, min 2 characters)!', true);
        setupInlineValidation('member_number', validateMemberNumber, 'member_number_error', 'Member number is required and must be a positive whole number (1, 2, 3, ...).', true);
        setupInlineValidation('head_email', validateEmail, 'head_email_error', 'Email is required. Please enter a valid email address (must contain @ and .com/.in/.org etc.)!', true);
        setupInlineValidation('head_phone', validatePhone, 'head_phone_error', 'Phone number is required. Invalid Indian mobile number. Expected 10 digits starting 6-9 (optional +91).', true);
        setupInlineValidation('head_dob', validateDateNotFuture, 'head_dob_error', 'Date of Birth is required and cannot be in future.', true);
        setupInlineValidation('head_address', validateAddress, 'head_address_error', 'Address is required (min 10 characters).', true);
        setupInlineValidation('join_date', validateDateNotFuture, 'join_date_error', 'Join date is required and cannot be in future.', true);
        setupInlineValidation('monthly_fee', validateMonthlyFee, 'monthly_fee_error', 'Monthly fee is required (must be a positive number).', true);
        setupInlineValidation('total_due', validatePositiveNumber, 'total_due_error', 'Total due is required (must be a positive number).', true);
        
        // Dropdown validations - ALL REQUIRED
        ...
        if (statusSelect) {
            ...
            if (!errorElement) {
                ...
                errorElement.id = 'member_status_error';
                errorElement.textContent = 'Member status is required. Please select member status.';
                errorElement.style.display = 'none';
                statusSelect.parentNode.appendChild(errorElement);
            }
        ...
        // Donation status validation - Required
        if (donationSelect) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // preload family members + their docs
    ...
    document.getElementById('sahakariForm').addEventListener('submit', function (e) {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    window.location.href = 'sahakari_details.php?id=' + (form.member_id.value || '<?php echo (int)$member['id']; ?>');
                }, 800);
            } else {
    ...
    function validateForm() {
        ...
        // Validate required fields for HEAD SECTION - ALL ARE REQUIRED
        const requiredValidations = [
            { id: 'head_name', fn: validateName, errorId: 'head_name_error', msg: 'Head of family name is required (only letters and spaces allowed, min 2 characters)!', required: true },
            { id: 'head_father_name', fn: validateFatherName, errorId: 'head_father_name_error', msg: 'Father\'s name is required (only letters and spaces allowed, min 2 characters)!', required: true },
            ...
            { id: 'head_email', fn: validateEmail, errorId: 'head_email_error', msg: 'Email is required. Please enter a valid email address (must contain @ and .com/.in/.org etc.)!', required: true },
            ...
            { id: 'join_date', fn: validateDateNotFuture, errorId: 'join_date_error', msg: 'Join date is required and cannot be in future.', required: true },
            { id: 'monthly_fee', fn: validateMonthlyFee, errorId: 'monthly_fee_error', msg: 'Monthly fee is required (must be a positive number).', required: true },
            { id: 'total_due', fn: validatePositiveNumber, errorId: 'total_due_error', msg: 'Total due is required (must be a positive number).', required: true }
        ];
        ...
        // Validate required dropdowns for HEAD SECTION - ALL ARE REQUIRED
        const genderSelect = document.getElementById('head_gender');
        if (genderSelect) {
        ...
        // Validate family members - NAME AND RELATIONSHIP ARE REQUIRED
        const cards = document.querySelectorAll('.family-card');
        ...
        // documents validation + duplicates
        const seen = new Set();
    ...
    document.getElementById('sahakari-manage-btn').addEventListener('click', () => {
        window.location.href = 'sahakari_management.php';
    });
