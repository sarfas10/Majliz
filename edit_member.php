...
function getNextMemberNumberForMahal($conn, $mahal_id) {
    ...
    try {
        ...
        $stmt->close();
        if ($res && $res['mx'] !== null) $next = ((int)$res['mx']) + 1;
    } catch (Throwable $e) {}
    return $next;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_member') {
    ...
    try {
        ...
        // total_family_members (i), monthly_donation_due (s), total_due (d), monthly_fee (d),
        ...
        foreach ($family_members as $fm) {
            ...
            $sqlChild = "INSERT INTO members
                (head_name, father_name, email, phone, dob, gender, status, occupation, address, mahal_id, member_number, join_date,
                 total_family_members, monthly_donation_due, total_due, monthly_fee, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
            $stmtChild = $conn->prepare($sqlChild);
            if (!$stmtChild) throw new Exception("Prepare failed (child member): " . $conn->error);

            $child_total_members = 1;

            // final correct type string:
            // 1-9: s x9, 10-11: i i, 12: s, 13: i, 14: s, 15: d, 16: d
            $typeChild = "sssssssssiisidd";
            ...
            if (isset($fm['documents']) && is_array($fm['documents']) && !empty($fm['documents'])) {
                foreach ($fm['documents'] as $d) {
                    ...
                    $doc_stmt2 = $conn->prepare("INSERT INTO member_documents
                        (member_id, family_member_id, owner_type, doc_type, doc_number, name_on_doc, issued_by, issued_on, expiry_on, notes, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
                    if (!$doc_stmt2) throw new Exception("Prepare failed when copying documents for '{$child_name}': " . $conn->error);
    ...
    <div id="app">
        ...
        <main class="main" id="main">
            <div class="top-row">
                ...
                <a href="user_details.php?id=<?php echo (int)$member['id']; ?>" class="btn btn-primary-form">← Back to Member</a>
            </div>

            <div class="container">
                <div id="alertBox" class="alert"></div>
                <div id="loadingBox" class="loading"><p>✏️ Updating member data...</p></div>
...
<div class="form-group">
    ...
    <div class="help">Required — appears next to member name.</div>
                            ...
                            <div class="form-group">
                                <label>Email</label>
                                <input type="email" name="head_email" id="head_email" value="<?php echo htmlspecialchars($member['email']); ?>" maxlength="120" inputmode="email" autocomplete="email">
    ...
    <div class="error-message" id="join_date_error">Date cannot be in future</div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="total_due" id="total_due" step="0.01" min="0" value="<?php echo htmlspecialchars(number_format((float)$member['total_due'], 2, '.', '')); ?>">
                                <div class="error-message" id="total_due_error">Must be a positive number</div>
                            </div>
                            <div class="form-group">
                                <label>Monthly Fee (₹ / month)</label>
                                <input type="number" name="monthly_fee" id="monthly_fee" step="0.01" min="0" value="<?php echo htmlspecialchars(number_format((float)($member['monthly_fee'] ?? 0), 2, '.', '')); ?>">
                    ...
                    <div class="form-section">
                        ...
                        <div class="help" style="margin-top:8px;">
                            If "Already an individual member" shows for someone, deleting them here will only remove them from this family list — their separate individual member record stays.
                        </div>
                    </div>
                    ...
                    <div class="actions">
                        <div class="actions-left">
                            <a href="user_details.php?id=<?php echo (int)$member['id']; ?>" class="btn btn-secondary-form">Cancel</a>
