...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_changes'])) {
    ...
    // Fetch current details_json for this request
    $sql = "SELECT details_json FROM cert_requests WHERE id = ? AND certificate_type = 'termination' LIMIT 1";
    ...
    $city_line         = trim((string)($_POST['city_line']         ?? ''));
    $taluk             = trim((string)($_POST['taluk']             ?? ''));
    $village_panchayat = trim((string)($_POST['village_panchayat'] ?? ''));
    $phone             = trim((string)($_POST['phone']             ?? ''));
    $member_number     = trim((string)($_POST['member_number']     ?? ''));
    $destination_mahal = trim((string)($_POST['destination_mahal'] ?? ''));
    $reg_number        = trim((string)($_POST['reg_number']        ?? ''));
    $reason_termination= trim((string)($_POST['reason_termination']?? ''));
    $pending_dues      = trim((string)($_POST['pending_dues']      ?? ''));

    if ($dob !== '') {
        try {
            $d = new DateTime($dob);
            $now = new DateTime();
            $ageCalc = $now->diff($d)->y;
            $age = (string)$ageCalc;
        } catch (Throwable $e) {
            // keep posted age
        }
    ...
    $details['certificate_title']      = $details['certificate_title'] ?? 'Membership Termination Certificate';
    $details['head_name']             = $head_name;
    $details['father_name']           = $father_name;
    $details['member_number']         = $member_number;
    $details['qualification']         = $qualification;
    $details['house_name']            = $house_name;
    $details['city_district_pincode'] = $city_line;
    $details['taluk']                 = $taluk;
    $details['village_panchayat']     = $village_panchayat;
    $details['address']               = $address_combined;
    $details['phone']                 = $phone;
    $details['date_of_birth']         = $dob;
    $details['age']                   = $age !== '' ? (int)$age : null;
    $details['destination_mahal']     = $destination_mahal;
    $details['reg_number']            = $reg_number;
    $details['reason_termination']    = $reason_termination;
    $details['pending_dues']          = $pending_dues;

    $details_json = json_encode($details, JSON_UNESCAPED_UNICODE);

    // Optional new member photo upload
    $member_photo_blob = null;
    if (!empty($_FILES['member_photo']['tmp_name']) && ($_FILES['member_photo']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK) {
        $member_photo_blob = file_get_contents($_FILES['member_photo']['tmp_name']);
    }

    // Build dynamic UPDATE query (only update member_photo if new one uploaded)
    $sqlUpd  = "UPDATE cert_requests SET details_json = ?, updated_at = NOW()";
    $types   = "s";
    $params  = [$details_json];

    if ($member_photo_blob !== null) {
        $sqlUpd  .= ", member_photo = ?";
        $types   .= "s";
        $params[] = $member_photo_blob;
    }

    $sqlUpd .= " WHERE id = ? AND certificate_type = 'termination' LIMIT 1";
    $types  .= "i";
    $params[] = $request_id;
    ...
    header("Location: termination.php?request_id={$request_id}&saved=1");
    exit();
}
...
/* --- fetch termination request details from cert_requests --- */
$request_id = isset($_GET['request_id']) ? (int)$_GET['request_id'] : 0;

$head_name = $father_name = $dob = $age = '';
$qualification = '';
$house_name = $city_line = $taluk = $village_panchayat = '';
$phone = $member_number = $destination_mahal = $reg_number = '';
$reason_termination = $pending_dues = '';
$family_members = [];
$member_img_preview = '';
...
if ($request_id > 0) {
    ...
    if ($r = $res->fetch_assoc()) {
        ...
        if (is_array($details)) {
            ...
            $ageVal            = $details['age']                   ?? null;
            $age               = $ageVal !== null && $ageVal !== '' ? (string)$ageVal : '';
            $qualification     = $details['qualification']         ?? '';
            $house_name        = $details['house_name']            ?? '';
            $city_line         = $details['city_district_pincode'] ?? '';
            $taluk             = $details['taluk']                 ?? '';
            $village_panchayat = $details['village_panchayat']     ?? '';
            $phone             = $details['phone']                 ?? '';
            $member_number     = $details['member_number']         ?? '';
            $destination_mahal = $details['destination_mahal']     ?? ($details['new_mahal_transferring_to'] ?? '');
            $reg_number        = $details['reg_number']            ?? '';
            $reason_termination= $details['reason_termination']    ?? '';
            $pending_dues      = $details['pending_dues']          ?? '';
            $family_members    = $details['family_members']        ?? [];
        }

        // Build member photo preview from BLOB (if present)
        if (!empty($r['member_photo'])) {
            $blob = $r['member_photo'];
            $mime = 'image/jpeg';
            if (strncmp($blob, "\x89PNG", 4) === 0) {
                $mime = 'image/png';
            }
            $member_img_preview = 'data:' . $mime . ';base64,' . base64_encode($blob);
        }
    }
    $stmt->close();
}
...
<head>
  ...
  <title>Termination Certificate — PDF Generator</title>
...
<body>
  ...
  <div id="app">
    ...
    <main class="main" id="main">
      ...
      <div class="content-area">
        <div class="wrap">
          <div class="card">
            <h1>Membership Termination Certificate — PDF Generator</h1>
            ...
            <div class="readonly-box">
              <strong><?php echo h($mahal_name); ?></strong><br>
              <span class="muted"><?php echo nl2br(h($mahal_address)); ?></span><br>
              <span class="muted">REG. NO: <?php echo h($mahal_reg); ?></span>
            </div>

            <form method="post" action="" enctype="multipart/form-data">
              <input type="hidden" name="request_id" value="<?php echo (int)$request_id; ?>">

              <div class="full"><h3>Member Details</h3></div>

              <!-- Member block: left = fields, right = photo -->
                  ...
                  <div class="grid">
                    <div>
                      ...
                      <input type="text" name="head_name" required
                             value="<?php echo h($head_name); ?>" />
                    </div>

                    <div>
                      <label>Father's Name *</label>
                      <input type="text" name="father_name" required
                             value="<?php echo h($father_name); ?>" />
                    </div>

                    <div>
                      <label>Date of Birth</label>
                      <input type="date" name="dob" id="dobInput"
                             value="<?php echo h($dob); ?>" />
                    </div>
                    ...
                    <div class="full">
                      ...
                      <input type="text" name="qualification"
                             value="<?php echo h($qualification); ?>" />
                    </div>

                    <div class="full">
                      <label>House Name</label>
                      <input type="text" name="house_name"
                             value="<?php echo h($house_name); ?>" />
                    </div>

                    <div class="full">
                      <label>City, District &amp; Pincode</label>
                      <input type="text" name="city_line"
                             placeholder="Eg: Calicut, Kozhikode 673001"
                             value="<?php echo h($city_line); ?>" />
                    </div>

                    <div class="full">
                      <label>Taluk</label>
                      <input type="text" name="taluk"
                             value="<?php echo h($taluk); ?>" />
                    </div>

                    <div class="full">
                      <label>Village, Panchayat</label>
                      <input type="text" name="village_panchayat"
                             value="<?php echo h($village_panchayat); ?>" />
                    </div>
                    ...
                    <div>
                      ...
                      <input type="text" name="member_number" readonly
                             value="<?php echo h($member_number); ?>" />
                    </div>
                  </div>
                </div>

                <div class="member-right">
                  <label>Passport size photo (JPG/PNG, max 2MB)</label>
                  <input type="file" name="member_photo" accept="image/jpeg,image/png" />
                  <div class="muted">Preview (not saved until you submit):</div>
                  <img
                    id="memberPreview"
                    class="img-preview"
                    alt="Member preview"
                    <?php if ($member_img_preview): ?>
                      src="<?php echo h($member_img_preview); ?>"
                      style="display:block;"
                    <?php else: ?>
                      style="display:none;"
                    <?php endif; ?>
              ...
              <div class="grid">
                <div class="full">
                  <label>Destination Mahal (Transfer To)</label>
                  <input type="text" name="destination_mahal"
                         value="<?php echo h($destination_mahal); ?>" />
                </div>

                <div class="full">
                  <label>Certificate Registration No. *</label>
                  <input type="text" name="reg_number" required
                         value="<?php echo h($reg_number); ?>" />
                </div>

                <div class="full">
                  <label>Reason for Termination *</label>
                  <textarea name="reason_termination" rows="2" required><?php
                      echo h($reason_termination);
                  ?></textarea>
                </div>

                <div class="full">
                  <label>Details Regarding Pending Dues</label>
                  <textarea name="pending_dues" rows="2"><?php
                      echo h($pending_dues);
                  ?></textarea>
                </div>

                <div class="full">
                  <h3>Family Members (from request)</h3>
                      ...
                      <tbody>
                        <?php foreach ($family_members as $idx => $fm): ?>
                          <tr>
                            ...
                            <td><?php echo h($fm['name'] ?? ''); ?></td>
                            <td><?php echo h($fm['relationship'] ?? ''); ?></td>
                            <td><?php echo isset($fm['age']) ? h((string)$fm['age']) : ''; ?></td>
                            <td><?php echo h($fm['occupation'] ?? ''); ?></td>
                          </tr>
                        <?php endforeach; ?>
                      </tbody>
                    </table>
                  <?php else: ?>
                    <div class="muted">No family members stored in this request.</div>
                  <?php endif; ?>
                </div>
              </div>
              ...
              <div style="margin-top:20px; display:flex; gap:10px;">
                ...
                <button class="btn btn-secondary" type="submit"
                        formaction="generate_termination_certificate.php"
  ...
  <script>
    ...
    function setupAgeAuto() {
      ...
      function updateAge() {
        ageInput.value = calcAgeFromDobStr(dobInput.value);
      }
    ...
    document.addEventListener('DOMContentLoaded', function () {
      setupPreview('member_photo', 'memberPreview');
      setupAgeAuto();
